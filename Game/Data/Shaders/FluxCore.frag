...
vec3 RayTrace(in vec2 fragCoord )
{
    ...
    } else if (localTime < t6)
    {
        ...
        float smootaaa = smoothstep(0.0, 1.0, saturate(alpha*1.8-0.1));
        animStructure = 1.0-smootaaa;
        fade = saturate(time);
        fade *= saturate(t6 - localTime);
        camPos = vec3(10.0, -0.95+smootaaa*1.0, 0.0);
        camPos.x -= alpha * 6.8;
        camUp=normalize(vec3(0,1.0-smootaaa,0.0+smootaaa));
        camLookat=vec3(0,-0.0,-0.0);
    }
#endif
...
void main()
{
    ...
    for (float i = 0.0; i < antialiasingSamples; i++)
    {
        ...
        localTime += Hash11(v21(gl_FragCoord.xy + seed)) * motionBlurLengthInSeconds;
        // Jitter the pixel position so we get antialiasing when we do multiple passes.
        vec2 jittered = gl_FragCoord.xy + vec2(
            Hash21(gl_FragCoord.xy + seed),
            Hash21(gl_FragCoord.xy*7.234567 + seed)
            );
        // don't antialias if only 1 sample.
        if (antialiasingSamples == 1.0) jittered = gl_FragCoord.xy;
